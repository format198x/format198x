...
use format198x_sinclair_zx_spectrum_scr::{self as scr, DecodeError, EncodeError};
...
fn golden_round_trips_byte_for_byte() {
    let decoded =
        format198x_sinclair_zx_spectrum_scr::decode(GOLDEN).expect("golden fixture decodes");
    let reencoded =
        format198x_sinclair_zx_spectrum_scr::encode(&decoded).expect("decoded screen re-encodes");
    assert_eq!(
        reencoded, GOLDEN,
...
fn malformed_input_never_panics() {
    for len in [0usize, 1, 6143, 6911, 6913, 8192] {
        ...
        assert!(
            format198x_sinclair_zx_spectrum_scr::decode(&bytes).is_err(),

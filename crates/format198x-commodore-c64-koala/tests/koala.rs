...
use format198x_commodore_c64_koala::{self as koala, DecodeError, EncodeError};
...
fn golden_round_trips_byte_for_byte() {
    let decoded = format198x_commodore_c64_koala::decode(GOLDEN).expect("decodes");
    let reencoded = format198x_commodore_c64_koala::encode(&decoded).expect("re-encodes");
...
fn wrong_load_address_is_rejected() {
    ...
    bytes[0] = 0x00;
    bytes[1] = 0x10;
    assert!(format198x_commodore_c64_koala::decode(&bytes).is_err());
}
...
fn malformed_input_never_panics() {
    for len in [0usize, 1, koala::FILE_LEN - 1, koala::FILE_LEN + 1, 20000] {
        ...
        assert!(
            format198x_commodore_c64_koala::decode(&bytes).is_err(),

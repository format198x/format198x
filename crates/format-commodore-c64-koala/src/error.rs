...
//! codec actually constructs — Koala validates its fixed length and its
//! $6000 load address, so it needs `WrongLength` and `BadMagic`, but never
//! the sibling variants (`Truncated`, `Unsupported`, `DimensionsTooLarge`,
//! `MissingChunk`, `Corrupt` on the decode side; `ValueOutOfRange` on the
//! encode side) that exist for the other codecs in that shared enum.
